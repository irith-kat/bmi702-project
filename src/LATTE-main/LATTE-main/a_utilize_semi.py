...
def get_data_from_csv(
    ...
    min_date = min(np.array(data_Time, dtype=np.int64))
    max_date = max(np.array(data_Time, dtype=np.int64))
    ...
    data_embedding_all = np.array(data_embedding_all, dtype=np.float64)
    ...
    for ID, count_key, count_all, label in zip(
        Patient_num, key_code_counts, all_code_counts, Y_label
    ):
        dic_label[str(ID)] = label
        if str(ID) not in dic_patient_utilization:
    ...
    for ID in dic_patient_utilization:
        ...
        silver_value = sigmod(
            (np.log10(count + 1) - alpha_silver * np.log10(1 + utilization))
            / temp_silver
        )
    ...
    for rowi in range(len(data_Time)):
        if True:
            ...
            if Patient_num[rowi] in target_patient or "ALL" in target_patient:
                if patient not in patients_total:
                    patient_col = np.ones(shape=(total_month, 1)) * int(
                        Patient_num[rowi]
                    )
                    ...
                    patient_time_col = np.concatenate(
                        (patient_time_col, label_col), axis=-1
                    )
                    patients_total[patient] = np.concatenate(
                        (
                            patient_time_col,
                            np.zeros(shape=(total_month, total_codes + 2)),
                        ),
                        -1,
                    )  ###########total_codes+2: phecodes
                if patient in patients_total:
                    index_row = int(
                        int(int(data_Time[rowi]) - min_date) / month_window
                    )  # 这里还是使用了month_window
                    patients_total[patient][
                        index_row, 5 : 5 + colums_max - colums_min
                    ] += data_array_all[rowi]
        ...
        for addi in window_offset:  # 由于range是半闭半开区间，只取到2，3，4三个值
            ...
            for rowi in range(len(data_Time)):
                ...
                if Patient_num[rowi] in target_patient or "ALL" in target_patient:
                    ...
                    if patient in patients_total:
                        index_row = int(
                            int(int(data_Time[rowi]) - min_date) / month_window_temp
                        )
                        patients_total[patient][
                            index_row, 5 : 5 + colums_max - colums_min
                        ] += data_array_all[rowi]
                        patients_total[patient][index_row, 4] = dic_patient_silver[
                            patient_raw
                        ]
                        patients_total[patient][index_row, 3] = dic_patient_silver[
                            patient_raw
                        ]
    ...
    for patient_i in numbers_total:
        ...
        for visit_i in range(rows):
            if (
                np.sum(data_i[visit_i, 5:]) > 0 and len(data_temp) < visit_maximum
            ):  # 仅仅记录有事件出现的点？为什么
                ...
                if (visit_i - neighbout_stress > 0) and (
                    visit_i + neighbout_stress < len(data_i)
                ):
                    if (
                        data_i[visit_i - neighbout_stress, 0]
                        == data_i[visit_i + neighbout_stress, 0]
                        and not data_i[visit_i - neighbout_stress, 2]
                        == data_i[visit_i + neighbout_stress, 2]
                    ):
        ...
        for add_i in range(
            visit_maximum - len(data_temp)
        ):  # 超过的部分用最后一个补足;这一处实际上没有发挥作用
    ...
    with open(dirr_save + save_name, "wb") as fid:
        pickle.dump(
            (
                np.array(data_total),
                np.array(label_total),
                np.array(patient_num_total),
                np.array(date_total),
                np.array(weight_total),
                np.array(data_embedding_all),
                np.tile(key_embedding, (colums_max - colums_min, 1)),
                silver_total,
            ),
            fid,
            protocol=4,
