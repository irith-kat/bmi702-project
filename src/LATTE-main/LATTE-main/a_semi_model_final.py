...
from a_Transformer import MultiHeadAttention
...
def Attention_train(
    ...
    epoch_silver=10,
    layers_incident="80",
    weights_prevalence=0.25,
    weights_unlabel=0.3,
    weights_constrastive=0.1,
    weight_smooth=0.1,
    weights_additional=0.1,
    ...
    with open(dirr_unlabel + filename_unlabel, "rb") as fid:
        (
            data_train_unlabel,
            y_train_unlabel,
            patient_num_train_unlabel,
            date_train_unlabel,
            weights_train_unlabel,
            data_embedding_unsuper,
            key_embedding_unlabel,
            silver_unsuper,
        ) = pickle.load(fid)
    ...
    print(
        "--------------------------patient_num_train.shape: ",
        np.array(patient_num_train).shape,
    )
    ...
    train_loss_classfication = tf.keras.metrics.Mean(
        name="train_smooth_loss_classfication"
    )
    ...
    train_constrastive_loss_MLP = tf.keras.metrics.Mean(
        name="train_constrastive_loss_MLP"
    )
    ...
    train_metric = tf.keras.metrics.AUC(
        name="train_auc",
    )
    ...
    def code_attention(input_seq, embedding):
        ...
        layer1 = layers.Dense(dim, activation="relu", name="code_attention_fc1")
        layer11 = layers.Dense(dim, activation="relu", name="code_attention_fc11")
        layer3 = layers.Dense(1, activation=None, name="code_attention_prediction")
        input_seq = layer1(input_seq)
        embedding = layer11(embedding)
        data_embedding = kops.concatenate([input_seq, embedding], axis=-1)
        data_embedding = layer3(data_embedding)
        data_embedding = kops.squeeze(data_embedding, axis=-1)
        data_embedding = kops.softmax(data_embedding, axis=-1)
        data_embedding = kops.expand_dims(data_embedding, axis=-2)
    ...
    def Model_prediction():
        ...
        inputs1_shuffle = layers.Input(
            shape=(
                max_lengh,
                feature_d,
            )
        )
        inputs_unsuper = layers.Input(
            shape=(
                max_lengh,
                feature_d,
            )
        )
        inputs_unsuper_masked = layers.Input(
            shape=(
                max_lengh,
                feature_d,
            )
        ...
        inputs_data_embedding_all = layers.Input(
            shape=(
                feature_d,
                embedding_d,
            )
        )
        inputs_data_embedding_unsuper = layers.Input(
            shape=(
                feature_d,
                embedding_d,
            )
        )
        inputs_keyfeature = layers.Input(
            shape=(
                feature_d,
                embedding_d,
            )
        )
        inputs_keyfeature_unsuper = layers.Input(
            shape=(
                feature_d,
                embedding_d,
            )
        ...
        attention_value = 10.0 * code_attention(
            inputs_data_embedding_all, inputs_keyfeature
        )
        bias_value = 0.1
        ...
        inputs_unsuper_reweight_mask = (
            attention_value + bias_value
        ) * inputs_unsuper_masked
        ##############the input counts is weighted

        inputs1_embedding_code_rw = kops.matmul(
            inputs1_reweight, inputs_data_embedding_all
        )

        ###################the re-weighted code utilization
        code_num = kops.sum(inputs1_reweight, axis=-1, keepdims=True) + 1
        code_num_unsuper = kops.sum(inputs_unsuper_reweight, axis=-1, keepdims=True) + 1
        code_num_unsuper_mask = (
            kops.sum(inputs_unsuper_reweight_mask, axis=-1, keepdims=True) + 1
        )
        code_num_shuffle = (
            kops.sum(inputs1_reweight_shuffle, axis=-1, keepdims=True) + 1
        )
        ###################the re-weighted code utilization

        ###########getting the normalized counts*embedding with the counts is re-weighted
        inputs1_embedding_ori = kops.matmul(inputs1_temp, inputs_data_embedding_all)
        inputs1_embedding = kops.matmul(
            inputs1_reweight / code_num, inputs_data_embedding_all
        )
        inputs1_embedding_shuffle = kops.matmul(
            inputs1_reweight_shuffle / code_num_shuffle, inputs_data_embedding_all
        )
        inputs_embedding_unsuper = kops.matmul(
            inputs_unsuper_reweight / code_num_unsuper, inputs_data_embedding_unsuper
        )
        inputs_embedding_unsuper_mask = kops.matmul(
            inputs_unsuper_reweight_mask / code_num_unsuper_mask,
        ...
        Prevalence_layer1 = layers.Dense(
            int(d_model_T), activation="relu", name="Binary_fcn1"
        )
        Prevalence_predictor = layers.Dense(1, activation=None, name="Binary_predictor")
        Prevalence_predictor_silver = layers.Dense(
            1, activation=None, name="Binary_predictor_silver"
        )
        ########using self-attention to learn visit imporance

        ####### getting Prevalence_prediction
        Prevalence_fcn, visit_weights = Attention_prevalence(
            inputs1_embedding, inputs1_embedding, inputs1_embedding, mask=None
        )
        Prevalence_fcn = Prevalence_layer1(kops.mean(Prevalence_fcn[:, :, :], axis=1))
        Prevalence_prediction = layers.Dropout(0.2)(Prevalence_fcn)
        Prevalence_prediction = Prevalence_predictor(Prevalence_prediction)

        Prevalence_fcn_unsuper, visit_weights_unsuper = Attention_prevalence(
            inputs_embedding_unsuper,
            inputs_embedding_unsuper,
            inputs_embedding_unsuper,
            None,
        )
        Prevalence_fcn_unsuper = Prevalence_layer1(
            kops.mean(Prevalence_fcn_unsuper[:, :, :], axis=1)
        )
        Prevalence_fcn_unsuper = layers.Dropout(0.2)(Prevalence_fcn_unsuper)
        Prevalence_prediction_unsuper = Prevalence_predictor(Prevalence_fcn_unsuper)
        Prevalence_prediction_silver = Prevalence_predictor_silver(
            Prevalence_fcn_unsuper
        )

        Prevalence_fcn_shuffle, visit_weights_shuffle = Attention_prevalence(
            inputs1_embedding_shuffle,
            inputs1_embedding_shuffle,
            inputs1_embedding_shuffle,
            None,
        )
        Prevalence_fcn_shuffle = Prevalence_layer1(
            kops.mean(Prevalence_fcn_shuffle[:, :, :], axis=1)
        )
        Prevalence_prediction_shffle = layers.Dropout(0.2)(Prevalence_fcn_shuffle)
        Prevalence_prediction_shffle = Prevalence_predictor(
            Prevalence_prediction_shffle
        )

        Prevalence_fcn_unsuper_mask, visit_weights_mask = Attention_prevalence(
            inputs_embedding_unsuper_mask,
            inputs_embedding_unsuper_mask,
            inputs_embedding_unsuper_mask,
            None,
        )
        Prevalence_fcn_unsuper_mask = Prevalence_layer1(
            kops.mean(Prevalence_fcn_unsuper_mask[:, :, :], axis=1)
        )
        Prevalence_prediction_unsuper_mask = layers.Dropout(0.2)(
            Prevalence_fcn_unsuper_mask
        )
        Prevalence_prediction_unsuper_mask = Prevalence_predictor(
            Prevalence_prediction_unsuper_mask
        )

        ##########getting the weights
        visit_weights_show = kops.mean(visit_weights[:, :, 0, :], axis=1)
        inputs1_embedding_code_visit_rw = (
            kops.expand_dims(kops.mean(visit_weights[:, :, 0, :], axis=1), axis=-1)
            * inputs1_reweight
        )
        ##########getting the weights

        visit_weights = (
            kops.expand_dims(kops.sum(visit_weights[:, 0, :, :], axis=1), -1) + 0.1
        )
        visit_weights_unsuper = (
            kops.expand_dims(kops.sum(visit_weights_unsuper[:, 0, :, :], axis=1), -1)
            + 0.1
        )
        visit_weights_mask = (
            kops.expand_dims(kops.sum(visit_weights_mask[:, 0, :, :], axis=1), -1) + 0.1
        )
        visit_weights_shuffle = (
            kops.expand_dims(kops.sum(visit_weights_shuffle[:, 0, :, :], axis=1), -1)
            + 0.1
        )
        ...
        representation_out_unsuper_mask = (
            inputs_embedding_unsuper_mask * visit_weights_mask
        )
        ...
        try:
            ...
            for layer_i in layes_num:
                ...
                representation_out_unsuper = GRU_Bidirectional(
                    representation_out_unsuper
                )
                representation_out_unsuper_mask = GRU_Bidirectional(
                    representation_out_unsuper_mask
                )
                representation_out_shuffle = GRU_Bidirectional(
                    representation_out_shuffle
                )
        except Exception:
            print(
                "----------------------------error of readding layers_incident: using default one layer with 80"
            )
        ...
        GRU_incident = tf.keras.layers.GRU(
            units=10, return_sequences=True, activation="relu"
        )
        _dropout_01 = layers.Dropout(0.1)
        representation_out = _dropout_01(representation_out)
        representation_out_unsuper = _dropout_01(representation_out_unsuper)
        representation_out_unsuper_mask = _dropout_01(representation_out_unsuper_mask)
        representation_out_shuffle = _dropout_01(representation_out_shuffle)
        ...
        inputs1_embedding_out = (
            representation_out  # tf.nn.dropout(representation_out, rate=0.0)
        )
        ...
        inputs_unsuper_embedding_out_mask = predictor_temporal(
        ...
        model = models.Model(
            inputs=[
                inputs1,
                inputs_data_embedding_all,
                inputs_unsuper,
                inputs_unsuper_masked,
                inputs_data_embedding_unsuper,
                inputs_keyfeature,
                inputs_keyfeature_unsuper,
                inputs1_shuffle,
            ],
            outputs=[
                inputs1_embedding_out,
                inputs_unsuper_embedding_out,
                representation_out,
                representation_out_unsuper,
                representation_out_unsuper_mask,
                ...
                visit_weights_show,
                inputs1_embedding_ori,
                inputs1_embedding_code_rw,
                inputs1_embedding_code_visit_rw,
                representation_out_shuffle,
                attention_value,
                Prevalence_fcn,
                Prevalence_fcn_shuffle,
        ...
        )  #  outputs_sque  outputs  outputs_fused
        return model
    ...
    def train_step(
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            weights_smooth_super = tf.cast(
                tf.greater(weights_smooth_super, 0.0), tf.float32
            )
            ...
            shuffled_weights_smooth_super = tf.gather(
                weights_smooth_super, shuffled_indices
            )
            shuffled_labels = tf.gather(labels, shuffled_indices)
            batch_num = data_unsuper.numpy().shape[0]
            if random.random() < 0.5:
                mask_unsuper = np.random.normal(
                    loc=1, scale=0.06, size=(batch_num, max_lengh, feature_d)
                )
            else:
                mask_unsuper = np.random.normal(
                    loc=1, scale=0.01, size=(batch_num, max_lengh, feature_d)
                )
            mask_dropout = []
            for iii in range(int(batch_num * max_lengh * feature_d)):
                if random.random() < 0.5:
                    ...
                    mask_dropout.append(1)
            mask_dropout = np.array(mask_dropout).reshape(
                (batch_num, max_lengh, feature_d)
            )
            ...
            (
                ...
                smooth_seq_mask,
                Prevalence_prediction,
                attention_value_codes,
                attention_value_visits,
                embedding_ori,
                embedding_code_rw,
                embedding_code_visit_rw,
                shuffled_smooth_seq,
                attention_value_code,
                Prevalence_fcn,
                Prevalence_fcn_shuffle,
                Prevalence_prediction_unsuper,
                Prevalence_prediction_unsuper_mask,
            ...
            prediction_incident_binary_unsuper = predictions_unsuper[
                0, weights_value_unsuper[0] - 1, 0
            ]
            prediction_incident_binary_unsuper = tf.expand_dims(
                prediction_incident_binary_unsuper, 0
            )
            prediction_incident_silver = prediction_silver[
                0, weights_value_unsuper[0] - 1, 0
            ]
            prediction_incident_silver = tf.expand_dims(prediction_incident_silver, 0)

            for rowi in range(1, len(weights_value)):
                prediction_incident_binary_temp = predictions[
                    rowi, weights_value[rowi] - 1, 0
                ]
                prediction_incident_binary_temp = tf.expand_dims(
                    prediction_incident_binary_temp, 0
                )
                prediction_incident_binary = tf.concat(
                    [prediction_incident_binary, prediction_incident_binary_temp],
                    axis=0,
                )

                prediction_incident_binary_temp = predictions_unsuper[
                    rowi, weights_value_unsuper[rowi] - 1, 0
                ]
                prediction_incident_binary_temp = tf.expand_dims(
                    prediction_incident_binary_temp, 0
                )
                prediction_incident_binary_unsuper = tf.concat(
                    [
                        prediction_incident_binary_unsuper,
                        prediction_incident_binary_temp,
                    ],
                    axis=0,
                )
                prediction_incident_binary_temp = prediction_silver[
                    rowi, weights_value_unsuper[rowi] - 1, 0
                ]
                prediction_incident_binary_temp = tf.expand_dims(
                    prediction_incident_binary_temp, 0
                ...
                prediction_incident_silver = tf.concat(
                    [prediction_incident_silver, prediction_incident_binary_temp],
                    axis=0,
                )
            prediction_MLP_total = (
                prediction_incident_binary.numpy()
            )  # keep as (batch,)
            # keep prediction_incident_silver as 1-D (batch,) — no expand_dims needed
            prediction_incident_silver = tf.nn.sigmoid(prediction_incident_silver)

            Prevalence_prediction = tf.squeeze(
                tf.nn.sigmoid(Prevalence_prediction), axis=-1
            )
            Prevalence_prediction_silver = tf.squeeze(
                tf.nn.sigmoid(Prevalence_prediction_silver), axis=-1
            )
            Prevalence_prediction_unsuper = tf.squeeze(
                tf.nn.sigmoid(Prevalence_prediction_unsuper), axis=-1
            )
            Prevalence_prediction_unsuper_mask = tf.squeeze(
                tf.nn.sigmoid(Prevalence_prediction_unsuper_mask), axis=-1
            )

            loss_MLP_incident = tf.reduce_mean(
                tf.abs((prediction_incident_binary) - (Prevalence_prediction))
            )
            loss_MLP_incident_unsuper = tf.reduce_mean(
                tf.abs(
                    (prediction_incident_binary_unsuper)
                    - (Prevalence_prediction_unsuper)
                )
            ...
            labels_MLP = tf.squeeze(labels_MLP, axis=-1)
            silver_unsuper = tf.reduce_mean(silver_unsuper, axis=1)
            silver_train = tf.reduce_mean(silver_train, axis=1)
            # print ("   AUC_silver:  ", roc_auc_score(y_true=labels_MLP.numpy(), y_score=silver_train.numpy()))

            labels_MLP_shuffle = tf.reduce_sum(shuffled_labels, axis=1)
            labels_MLP_shuffle = tf.cast(
                tf.greater_equal(labels_MLP_shuffle, 1.0), tf.float32
            )
            labels_MLP_shuffle = tf.squeeze(labels_MLP_shuffle, axis=-1)
            loss_binary_MLP = tf.keras.losses.binary_crossentropy(
                labels_MLP, Prevalence_prediction
            )
            loss_binary_MLP = tf.reduce_mean(loss_binary_MLP)
            loss_binary_MLP_silver = tf.keras.losses.binary_crossentropy(
                silver_unsuper, Prevalence_prediction_silver
            )
            loss_binary_MLP_silver = tf.reduce_mean(loss_binary_MLP_silver)
            loss_incident_silver = tf.keras.losses.binary_crossentropy(
                silver_unsuper, prediction_incident_silver
            )
            loss_incident_silver = tf.reduce_mean(loss_incident_silver)

            loss_consistency_MLP = tf.reduce_mean(
                tf.square(
                    Prevalence_prediction_unsuper - Prevalence_prediction_unsuper_mask
                )
            )
            loss_consistency = tf.reduce_sum(
                tf.abs(smooth_seq_unsuper - smooth_seq_mask)
                * tf.expand_dims(weights_unsuper, -1)
            ) / (tf.reduce_sum(weights_unsuper))

            ########smoonth loss as the non-decreasing loss
            smooth_loss = tf.abs(
                smooth_seq[:, 0:-1, :] - smooth_seq[:, 1:, :]
            ) * tf.expand_dims(weights_smooth_super[:, 1:], -1)

            smooth_loss_unsuper = tf.abs(
                smooth_seq_unsuper[:, 0:-1, :] - smooth_seq_unsuper[:, 1:, :]
            ) * tf.expand_dims(weights_unsuper[:, 1:], -1)

            smooth_loss_mask = tf.abs(
                smooth_seq_mask[:, 0:-1, :] - smooth_seq_mask[:, 1:, :]
            ) * tf.expand_dims(weights_unsuper[:, 1:], -1)
            ...
            entropy_unsuper = (
                -tf.reduce_sum(
                    predictions_unsuper * tf.math.log(predictions_unsuper)
                    + (1 - predictions_unsuper + 1e-10)
                    * tf.math.log(1 - predictions_unsuper + 1e-10)
                )
                * weights_unsuper
                / (tf.reduce_sum(weights_unsuper))
            )
            ...
            entropy_unsuper_MLP = -tf.reduce_mean(
                Prevalence_prediction_unsuper
                * tf.math.log(Prevalence_prediction_unsuper)
                + (1 - Prevalence_prediction_unsuper + 1e-10)
                * tf.math.log(1 - Prevalence_prediction_unsuper + 1e-10)
            )

            entropy_unsuper_MLP_mask = -tf.reduce_mean(
                Prevalence_prediction_unsuper_mask
                * tf.math.log(Prevalence_prediction_unsuper_mask)
                + (1 - Prevalence_prediction_unsuper_mask + 1e-10)
                * tf.math.log(1 - Prevalence_prediction_unsuper_mask + 1e-10)
            )

            ########constractive loss################
            features_half_batch = tf.reduce_sum(
                tf.abs(smooth_seq - shuffled_smooth_seq), axis=-1
            )
            weights_half_batch = weights_smooth_super + shuffled_weights_smooth_super
            weights_half_batch = tf.cast(
                tf.greater(weights_half_batch, 1.5), tf.float32
            )
            Y_half_batch = tf.reduce_mean(abs(labels - shuffled_labels), axis=-1)

            distance_same = (
                (1 - Y_half_batch)
                * tf.maximum(features_half_batch - threshold_embedding_same, 0.0)
                * weights_half_batch
            )
            distance_differ = (
                Y_half_batch
                * tf.maximum(threshold_embedding - features_half_batch, 0.0)
                * weights_half_batch
            )
            distance_constastive = distance_same + distance_differ
            distance_constastive = tf.reduce_sum(distance_constastive) / tf.reduce_sum(
                weights_half_batch
            )

            #################constrastive loss of MLP prediction##########
            features_half_batch = tf.reduce_sum(
                tf.abs(Prevalence_fcn - Prevalence_fcn_shuffle), axis=-1
            )
            Y_half_batch = abs(labels_MLP - labels_MLP_shuffle)
            # print("features_half_batch MLP: ", features_half_batch.numpy().shape)
            # print("Y_half_batch MLP: ", Y_half_batch.numpy().shape)
            distance_same = (1 - Y_half_batch) * tf.maximum(
                features_half_batch - threshold_MLP_same, 0.0
            )
            distance_differ = Y_half_batch * tf.maximum(
                threshold_MLP - features_half_batch, 0.0
            )
            ...
            loss_incident = (
                tf.reduce_sum(loss_incident, name="sigmoid_losses") / weights_sum
            )
            smooth_loss = tf.reduce_sum(smooth_loss, name="smooth_loss") / weights_sum
            smooth_loss_unsuper = (
                tf.reduce_sum(smooth_loss_unsuper, name="smooth_loss_unsuper")
                / weights_sum_unsuper
            )
            smooth_loss_mask = (
                tf.reduce_sum(smooth_loss_mask, name="smooth_loss_mask")
                / weights_sum_unsuper
            )

            loss_MLP = (
                loss_binary_MLP
                + distance_constastive_MLP * weights_constrastive
                + loss_consistency_MLP * smooth_weight_in
                + (entropy_unsuper_MLP) * smooth_weight_in * 0.5
            )
            loss_temporal = (
                loss_incident
                + (smooth_loss_unsuper + smooth_loss_mask + smooth_loss * 0.5)
                * weight_smooth
                * 0.5
                + distance_constastive * weights_constrastive
                + loss_consistency * smooth_weight_in
            )  # +entropy_unsuper*smooth_weight_in*0.5
            ...
            loss_total = (
                loss_MLP * weights_prevalence
                + loss_temporal
                + (loss_binary_MLP_silver * weights_prevalence + loss_incident_silver)
                * weights_unlabel
                + loss_MLP_incident * smooth_weight_in
            )
            loss_silver = (
                loss_binary_MLP_silver * weights_prevalence + loss_incident_silver
            )
        ...
        if not flag_silver:
            ...
            optimizer_silver.apply_gradients(
                grads_and_vars=zip(gradients, all_variables)
            )
        ...
        train_MLP_entropy_unsuper.update_state(
            (entropy_unsuper + entropy_unsuper_MLP + entropy_unsuper_MLP_mask / 3.0)
        )
        train_MLP_consistency.update_state(
            (loss_consistency + loss_consistency_MLP) * 0.5
        )
        train_loss_silver.update_state(loss_silver)

        Prevalence_prediction_final = (
            (Prevalence_prediction.numpy() + np.array(prediction_MLP_total)) / 2.0
        )  # (Prevalence_prediction.numpy() + np.array(prediction_MLP_total)) / 2.0
        return (
            loss_incident.numpy(),
            predictions.numpy(),
            Prevalence_prediction_final,
            labels_MLP.numpy(),
            attention_value_codes.numpy(),
            attention_value_visits.numpy(),
            embedding_ori.numpy(),
            embedding_code_rw.numpy(),
            embedding_code_visit_rw.numpy(),
            smooth_seq.numpy(),
            attention_value_code.numpy(),
            Prevalence_prediction.numpy(),
            np.array(prediction_MLP_total),
        )
    ...
    def valid_step(
        ...
        for rowi in range(1, len(weights_value)):
            ...
            prediction_incident_binary = tf.concat(
                [prediction_incident_binary, prediction_incident_binary_temp], axis=0
            )
        prediction_MLP_total = prediction_incident_binary.numpy()  # keep as (batch,)
        Prevalence_prediction = tf.squeeze(
            tf.nn.sigmoid(Prevalence_prediction), axis=-1
        )
        labels = tf.cast(labels, tf.float32)
        ...
        labels_MLP = tf.squeeze(
            tf.cast(tf.greater(labels_MLP, 0.0), tf.float32), axis=-1
        )
        weights = tf.cast(weights, tf.float32)
        ...
        Prevalence_prediction_final = (
            Prevalence_prediction.numpy() + np.array(prediction_MLP_total)
        ) / 2.0  # (Prevalence_prediction.numpy()+np.array(prediction_MLP_total))/2.0
        return (
            loss.numpy(),
            predictions.numpy(),
            Prevalence_prediction_final,
    ...
    def train_model(model, ds_train, ds_valid, epochs):
        ...
        while epoch_num < epochs:
            ...
            if len(AUC_MLP_test_total) > 9:
                if np.mean(AUC_MLP_test_total[-6:-3]) > np.mean(
                    AUC_MLP_test_total[-3:]
                ):
                    epoch_num += 2
            if epoch_num < 5:
                ...
                smooth_weight_temp = (
                    0.03 + weights_additional * (epoch_num - 1) / epochs
                )  #  *(epoch_num-1)/epochs

            if epoch_num < epoch_silver:
                flag_silver = True
                print(
                    "-------------------------------pre-training using silver labels-----------"
                )
            else:
                flag_silver = False
                print(
                    "-------------------------------joint training with silver labels-----------"
                )
            ...
            for (
                ...
                date_train,
                weights_train,
                data_embedding_all_temp,
                data_embedding_unsuper_temp,
                data_unsuper,
                weights_unsuper,
                keyfeature_train,
                keyfeature_unsuper,
                silver_train_batch,
                silver_unsuper_batch,
            ) in ds_train:
                (
                    loss_out,
                    predictions,
                ...
                if i_number == 0:
                    ...
                    predictions_train_total = np.concatenate(
                        (predictions_train_total, np.array(predictions)), axis=0
                    )
                    labels_train_total = np.concatenate(
                        (labels_train_total, np.array(y_train)), axis=0
                    )
                    weights_train_total = np.concatenate(
                        (weights_train_total, np.array(weights_train)), axis=0
                    )
                    Prevalence_prediction_total = np.concatenate(
                        (Prevalence_prediction_total, np.array(Prevalence_prediction)),
                        axis=0,
                    )
                    labels_MLP_total = np.concatenate(
                        (labels_MLP_total, np.array(labels_MLP)), axis=0
                    )

                    embedding_ori_train = np.concatenate(
                        (embedding_ori_train, np.array(embedding_ori)), axis=0
                    )
                    labels_MLP_total_train = np.concatenate(
                        (labels_MLP_total_train, np.array(labels_MLP)), axis=0
                    )
            ...
            if epoch_num % epoch_show == 0 or epoch_num > epochs - 2:
                for (
                    ...
                    date_test,
                    weights_test,
                    data_embedding_test_temp,
                    data_embedding_test_temp_temp,
                    data_unsuper,
                    weights_unsuper,
                    keyfeature_test,
                    keyfeature_test_unsuper,
                ) in ds_valid:
                    (
                        loss_out,
                        predictions,
                    ...
                    if i_number == 0:
                        ...
                        Prevalence_prediction_total_test = np.array(
                            Prevalence_prediction
                        )
                        Prevalence_prediction_total_test_MLP_ONLY = np.array(
                            binary_MLP_only
                        )
                        Prevalence_prediction_total_test_temporal_only = np.array(
                            binary_temporal_only
                        )
                        ...
                        predictions_test_total = np.concatenate(
                            (predictions_test_total, np.array(predictions)), axis=0
                        )
                        labels_test_total = np.concatenate(
                            (labels_test_total, np.array(y_test)), axis=0
                        )
                        weights_test_total = np.concatenate(
                            (weights_test_total, np.array(weights_test)), axis=0
                        )
                        date_test_total = np.concatenate(
                            (date_test_total, np.array(date_test)), axis=0
                        )
                        patient_num_test_total = np.concatenate(
                            (patient_num_test_total, np.array(patient_num_test)), axis=0
                        )
                        Prevalence_prediction_total_test = np.concatenate(
                            (
                                Prevalence_prediction_total_test,
                                np.array(Prevalence_prediction),
                            ),
                            axis=0,
                        )
                        ...
                        Prevalence_prediction_total_test_temporal_only = np.concatenate(
                            (
                                Prevalence_prediction_total_test_temporal_only,
                                np.array(binary_temporal_only),
                            ),
                            axis=0,
                        )

                        labels_MLP_total_test = np.concatenate(
                            (labels_MLP_total_test, np.array(labels_MLP)), axis=0
                        )
                        attention_value_visit_test = np.concatenate(
                            (
                                attention_value_visit_test,
                                np.array(attention_value_visits),
                            ),
                            axis=0,
                        )
                        attention_value_code_test = np.concatenate(
                            (
                                attention_value_code_test,
                                np.array(attention_value_codes),
                            ),
                            axis=0,
                        )

                        embedding_ori_test = np.concatenate(
                            (embedding_ori_test, np.array(embedding_ori)), axis=0
                        )
                        embedding_code_rw_test = np.concatenate(
                            (embedding_code_rw_test, np.array(embedding_code_rw)),
                            axis=0,
                        )
                        embedding_code_visit_rw_test = np.concatenate(
                            (
                                embedding_code_visit_rw_test,
                                np.array(embedding_code_visit_rw),
                            ),
                            axis=0,
                        )
                ...
                if flag_save_attention > 0:
                    ...
                    if epoch_num % 10 == 0 or epoch_num > epochs - 2:
                        savefile_name = (
                            filename_save
                            + flag_save
                            + "Attenation_value_patient_visit_code_prediction_label_weight_test.pkl"
                        )
                        ...
                        savefile_name = (
                            filename_save
                            + flag_save
                            + "_embedding_patient_ori_codeRW_codeVistRW_hiddenFCN_label_test.pkl"
                        )
                        with open(dirr_save + savefile_name, "wb") as fid:
                            pickle.dump(
                                (
                        ...
                        savefile_name = (
                            filename_save
                            + flag_save
                            + "_embedding_patient_ori_hiddenFCN_label_train.pkl"
                        )
                        with open(dirr_save + savefile_name, "wb") as fid:
                            pickle.dump(
                                (
            ...
            if epoch_num % epoch_show == 0 or epoch_num > epochs - 2:
                ...
                score_get_train = np.array(
                    predictions_train_total, dtype=float
                ).reshape((-1, 1))
                weights_train = np.array(weights_train_total, dtype=int).reshape(
                    (-1, 1)
                )

                try:
                    AUC_train = roc_auc_score(
                        y_true=y_true_get_train,
                        y_score=score_get_train,
                        sample_weight=weights_train,
                        average="macro",
                    )

                    AUC_MLP_train = roc_auc_score(
                        y_true=labels_MLP_total,
                    ...
                    AUC_MLP_train = 0
                    print(
                        "-----------------------------------------prevelance AUC evaluation error--------------"
                    )
                try:
                    if flag_prediction > 0:
                        ...
                        AUC_MLP_test = roc_auc_score(
                            y_true=labels_MLP_total_test,
                            y_score=Prevalence_prediction_total_test,
                            average="macro",
                        )
                        AUC_MLP_test_MLP_only = roc_auc_score(
                            y_true=labels_MLP_total_test,
                            y_score=Prevalence_prediction_total_test_MLP_ONLY,
                            average="macro",
                        )
                        AUC_MLP_test_incident_only = roc_auc_score(
                            y_true=labels_MLP_total_test,
                    ...
                    print(
                        "-----------------------------------------incident AUC evaluation error--------------"
                    )
                AUC_MLP_test_total.append(AUC_MLP_test)
                Prevalence_prediction_total_binary = np.array(
                    np.greater(Prevalence_prediction_total, threshold), dtype=np.int32
                )
                Prevalence_prediction_total_test_binary = np.array(
                    np.greater(Prevalence_prediction_total_test, threshold),
                    dtype=np.int32,
                )
                ...
                # False negative rate
                FNR = fn / (tp + fn)
                acc_train_MLP = accuracy_score(
                    labels_MLP_total, Prevalence_prediction_total_binary
                )
                acc_test_MLP = accuracy_score(
                    labels_MLP_total_test, Prevalence_prediction_total_test_binary
                )
                logs = (
                    "---AUC_B_train: {}, AUC_B_comb:{} ,AUC_B_TF_only:{},AUC_B_incident_only:{} "
                    "PPV_MLP:{},"
                    "specif_B:{},"
                    "sensi_B:{},"
                ...
                tf.print(
                    tf.strings.format(
                        ...
                        (
                            AUC_MLP_train,
                            AUC_MLP_test,
                            AUC_MLP_test_MLP_only,
                            AUC_MLP_test_incident_only,
                            PPV_test_MLP,
                            specificity_test_MLP,
                            sensitivity_test_MLP,
                            NPV_test_MLP,
                            train_constrastive_loss_MLP.result(),
                            train_MLP_entropy_unsuper.result(),
                            train_MLP_consistency.result(),
                            train_MLP_incident.result(),
                            train_loss_silver.result(),
                        ),
                    )
                )

                labels_test_total = np.array(labels_test_total).reshape((-1, 1))
                predictions_test_total = np.array(predictions_test_total).reshape(
                    (-1, 1)
                )
                weights_test_total = np.array(weights_test_total).reshape((-1, 1))
                date_test_total = np.array(date_test_total).reshape((-1, 1))
                patient_num_test_total = np.array(patient_num_test_total).reshape(
                    (-1, 1)
                )
                y_true_get_test = []
                score_get_test = []
                ...
                for samplei in range(len(labels_test_total)):
                    if int(weights_test_total[samplei].flat[0]) > 0:
                ...
                score_get_test_binary = np.array(
                    np.greater(score_get_test, threshold), dtype=np.int32
                )
                ...
                logs = (
                    "---Epoch={},Loss:{}, Loss_binary:{}  Loss_smooth:{},"
                ...
                tf.print(
                    tf.strings.format(
                        ...
                        (
                            epoch_num,
                            train_loss.result(),
                            train_loss_classfication.result(),
                            train_smooth_loss.result(),
                            train_smooth_loss_unsuper.result(),
                            train_constrastive_loss.result(),
                            AUC_train,
                            AUC_test,
                            f_1,
                            PPV,
                ...
                if epoch_num > epochs - 2:
                    if True:
                        ...
                        dataframe = pd.DataFrame(
                            {
                                "Patient_num": patient_num_test_total2,
                        ...
                        savename_weights = (
                            dirr_save + "_" + filename_save + "_code_weights.csv"
                        )
                    ...
                    if True:
                        ...
                        print(
                            "len(labels_MLP_total_test): ",
                            np.array(labels_MLP_total_test).shape,
                        )
                        dataframe = pd.DataFrame(
                            {
                                "Predition": np.array(
                                ...
                                "Y": np.array((labels_MLP_total_test)).reshape(
                                    len(labels_MLP_total_test),
                                ),
                            }
                        )
                        ...
                        print(
                            "---------saving ends successfully--- ",
                            dirr_save,
                            ": ",
                            filename_save,
                        )

                if epoch_num > epochs - 2 and not flag_save_finish:
                    flag_save_finish = True
                    if True:
                        f = open(
                            dirr_save + filename_save + "_incident_evaluation.txt", "a"
                        )
                        f.write(
                            "Threshold value=0.5 AUC_train :%4f , NPV:%4f ,Specificity:%4f, F1:%4f ,"
                    ...
                    if True:
                        f = open(
                            dirr_save + filename_save + "_prevalence_evaluation.txt",
                            "a",
                        )
                        f.write(
                            " Threshold value=0.5 AUC_train :%4f , ACC_train:%4f, ,FPR:%4f, FNR:%4f, NPV_MLP:%4f "
            ...
            train_smooth_loss_unsuper.reset_state()
            train_keyfeature_loss.reset_state()
            train_constrastive_loss.reset_state()
            train_constrastive_loss_MLP.reset_state()
            train_MLP_entropy_unsuper.reset_state()
            train_MLP_consistency.reset_state()
            train_MLP_incident.reset_state()
            train_loss_silver.reset_state()

    model = Model_prediction()
    savename_model = dirr_save + filename_save + "_model.keras"
