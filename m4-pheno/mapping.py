...
def rollup_cpt_to_ccs(df: pd.DataFrame, cpt_column: str, mapping_file: str = "CCS_Services_Procedures_v2025-1.csv") -> pd.DataFrame:
    """
    ...
    df_result = df.copy()
    df_result[cpt_column] = df_result[cpt_column].astype(str).str.strip().str.upper().str.zfill(5)
    ...
    def find_ccs(code):
        ...
        match = mapping_df[
            (mapping_df['start_code'] <= code) & 
            (mapping_df['end_code'] >= code)
        ]
        
        if not match.empty:
            # Guidelines state specific categories take precedence over general ones [cite: 818]
            return match.iloc[0]['ccs_category'], match.iloc[0]['ccs_category_description']
        return None, None
    ...
    df_result[['ccs_category', 'ccs_description']] = pd.DataFrame(results.tolist(), index=df.index)
